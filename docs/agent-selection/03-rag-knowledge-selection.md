...
> 具体组件怎么选看 [检索组件选型](/agent-selection/16-retrieval-patterns)。具体产品怎么选看 [向量数据库选型](/agent-selection/15-vector-database-selection) 和 [Embedding 模型选型](/agent-selection/14-embedding-models)。如果是选 RAG 平台（RAGFlow、Dify 等）还是自建 pipeline，看 [RAG 平台与方案选型](/agent-selection/05-rag-platforms)。
