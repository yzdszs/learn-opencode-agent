...
> **数据权限的完整实现**见 [企业知识库权限过滤怎么设计](/agent-selection/18-enterprise-knowledge-permission)，包括 chunk 级 ACL metadata 设计、RBAC/LDAP 集成、行级安全模式和大规模 filter 性能影响。本文聚焦工具权限、操作审批和整体安全模型。
