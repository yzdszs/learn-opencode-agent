...
export const contextMemoryExperiment: Experiment = {
  ...
  title: '上下文与记忆流',
  summary: '观察当前输入、短期上下文、长期记忆与检索证据如何被筛选、压缩和标注来源，再组装进下一次模型调用。',
  kind: 'context-memory-flow',
  steps: [
    {
      id: 'turn-enters-window',
      title: 'Current Turn',
      description: '新一轮输入先进入短期上下文，和最近对话一起构成本次推理起点，同时标记哪些旧约束仍然有效。',
      activeNodes: ['turn', 'window'],
      activePaths: ['turn-window'],
      packet: { from: 'turn', to: 'window', label: 'turn' },
      traceEvents: [
        { id: 'turn-captured', type: 'input', title: '捕获当前轮次', detail: '接收用户目标和最近消息，识别仍然有效的任务约束，作为本轮推理起点。', status: 'done' },
        { id: 'stale-context-marked', type: 'thinking', title: '标记过期上下文', detail: '把已完成、被用户否定或不再相关的信息标为低优先级，避免旧信息污染本轮决策。', status: 'active' },
      ],
    },
    {
      id: 'retrieve-evidence',
      title: 'Retrieve Evidence',
      description: '系统根据当前目标触发检索，把短期上下文转成更窄的问题，只寻找当前决策缺失的长期证据。',
      activeNodes: ['window', 'retriever'],
      activePaths: ['window-retriever'],
      packet: { from: 'window', to: 'retriever', label: 'query' },
      traceEvents: [
        { id: 'query-built', type: 'thinking', title: '生成检索查询', detail: '推断检索意图，生成查询词，定位需要从长期记忆中补齐的信息缺口。', status: 'done' },
        { id: 'retrieval-scope-limited', type: 'thinking', title: '限制检索范围', detail: '只检索与当前目标、项目或用户偏好有关的记忆，避免把无关历史塞回上下文。', status: 'active' },
      ],
    },
    {
      id: 'memory-hit',
      title: 'Memory Hit',
      description: '检索器命中长期记忆后，只把与当前问题有关且有来源的事实片段带回当前上下文。',
      activeNodes: ['retriever', 'memory'],
      activePaths: ['retriever-memory'],
      packet: { from: 'retriever', to: 'memory', label: 'hit' },
      traceEvents: [
        { id: 'memory-returned', type: 'observation', title: '召回长期记忆', detail: '长期记忆返回命中来源和相关度评分，提取可引用的证据片段注入当前上下文。', status: 'done' },
        { id: 'memory-not-trusted-blindly', type: 'observation', title: '保持候选状态', detail: '召回记忆先作为候选证据，仍需结合当前文件、命令或用户最新指令判断是否适用。', status: 'active' },
      ],
    },
    {
      id: 'compact-context',
      title: 'Compact Context',
      description: '旧上下文和召回证据被压缩，保留目标、约束、事实来源和未完成事项，避免无关历史挤占窗口。',
      activeNodes: ['memory', 'compact'],
      activePaths: ['memory-compact'],
      packet: { from: 'memory', to: 'compact', label: 'facts' },
      traceEvents: [
        { id: 'context-compacted', type: 'repair', title: '压缩上下文', detail: '过滤低价值历史噪声，保留关键事实，生成压缩后的上下文快照。', status: 'done' },
        { id: 'source-tags-kept', type: 'repair', title: '保留来源标签', detail: '压缩时保留事实来源和可信度标记，防止摘要把猜测改写成事实。', status: 'active' },
      ],
    },
    {
      id: 'assemble-prompt',
      title: 'Assemble Prompt',
      description: '短期状态与压缩记忆合并，形成下一次模型调用所需的最小有效输入，并明确哪些证据来自记忆。',
      activeNodes: ['window', 'compact', 'prompt'],
      activePaths: ['compact-prompt', 'window-prompt'],
      packet: { from: 'compact', to: 'prompt', label: 'ctx' },
      traceEvents: [
        { id: 'prompt-ready', type: 'output', title: '组装 Prompt', detail: '组装最终上下文包，标注证据来源，输出下一次模型调用所需的最小有效输入。', status: 'done' },
        { id: 'context-budget-set', type: 'output', title: '控制上下文预算', detail: '把短期上下文、长期记忆和证据片段限制在可控比例，给模型保留推理空间。', status: 'active' },
      ],
    },
  ],
}
